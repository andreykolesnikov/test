...
# Автоматическое определение внешнего интерфейса с фолбэком на eth0
EXT_IF=$(ip route | grep default | awk '{print $5}' | head -1 || echo "eth0")
...
# === УЛУЧШЕНИЕ: Функция настройки NAT и IP Forwarding ===
setup_nat_forwarding() {
    echo -e "${GREEN}Настройка IP forwarding и NAT...${NC}"
    ...
    # NAT для всех режимов
    iptables -t nat -A POSTROUTING -s 10.2.0.0/24 ! -d 10.2.0.0/24 -o "$EXT_IF" -j MASQUERADE
    ...
    echo -e "${GREEN}NAT и forwarding настроены на интерфейсе: $EXT_IF${NC}"
}

# === УЛУЧШЕНИЕ: Функция настройки UFW ===
setup_ufw() {
    if command -v ufw &> /dev/null; then
        ...
        echo -e "${YELLOW}UFW не установлен, пропускаем настройку брандмауэра.${NC}"
    fi
}
...
install_dwg() {
    ...
    case $setup_choice in
        1) # DWG-CLI
            ...
            # === УЛУЧШЕНИЕ: Замена eth0 на $EXT_IF и использование двойных кавычек ===
...
EOF
    ...
    # === УЛУЧШЕНИЕ: Замена конфигурации AdGuardHome на более безопасную ===
...
EOF
    ...
    # === УЛУЧШЕНИЕ: Вызов функций настройки перед запуском контейнеров ===
    setup_nat_forwarding
    setup_ufw
...
show_info() {
    ...
    case $VERSION in
        cli)
            ...
            echo -e "${BLUE}AdGuardHome доступен через VPN: http://10.2.0.100 или https://10.2.0.100 (самоподписанный сертификат)${NC}"
        ...
        dark)
            ...
            echo -e "${BLUE}AdGuardHome через VPN: http://10.2.0.100 или https://10.2.0.100 (самоподписанный сертификат)${NC}"
            echo -e "${GREEN}Логин AdGuardHome: $adguard_user${NC}"
            echo -e "${GREEN}Пароль AdGuardHome: $adguard_password${NC}"
            ;;
        amnezia)
    ...
    # === УЛУЧШЕНИЕ: Добавление дополнительной информации ===
    echo -e "${GREEN}Внешний интерфейс:${NC} $EXT_IF"
    echo -e "${GREEN}NAT и forwarding:${NC} настроены"
}
...
status() {
    if [ -f "$WORK_DIR/docker-compose.yml" ]; then
        ...
        echo -e "${GREEN}External Interface:${NC} $EXT_IF"
        echo -e "${GREEN}Config Path:${NC} $CONF_DIR/AdGuardHome.yaml"
        if [ "$VERSION" == "ui" ] || [ "$VERSION" == "dark" ] || [ "$VERSION" == "amnezia" ]; then
            local port_from_compose
            port_from_compose=$(grep -A 5 "wg-easy:" "$WORK_DIR/docker-compose.yml" | grep 'ports:' -A 2 | grep 'tcp' | awk -F: '{print $1}' | sed 's/"//g' | tr -d '[:space:]')
            if [ -z "$port_from_compose" ]; then
                port_from_compose=$(grep -A 5 "amnezia-wg-easy:" "$WORK_DIR/docker-compose.yml" | grep 'ports:' -A 2 | grep 'tcp' | awk -F: '{print $1}' | sed 's/"//g' | tr -d '[:space:]')
            fi
            if [ -z "$port_from_compose" ]; then
                 port_from_compose="51821" # Fallback for dark mode
            fi
            echo -e "${GREEN}WireGuard Web UI:${NC} http://$MYHOST_IP:$port_from_compose"
        fi
...
version() {
    ...
    echo -e "${YELLOW}Скрипт версии: beta2 (с улучшениями)${NC}"
}
